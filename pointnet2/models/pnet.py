...
class Pnet2Stage(nn.Module):
    def __init__(self, mlp1, mlp2, bn=True, remove_last_activation=True):
        super().__init__()
        self.mlp1 = Mlp_plus_t_emb(mlp_spec=mlp1, bn=bn, t_dim=0, include_t=False,
    ...
    def forward(self, x):
        '''进行一些简单的mlp,转换特征, x(B, 4, N), global_feature(B, 1024)'''
        x_feature = x.unsqueeze(-1)    #(B, mlp1[0], num_points, 1)
        feature = self.mlp1(x_feature)      #(B, mlp[2], num_points, 1)
        global_feature = F.max_pool2d(feature, kernel_size=[feature.size(2), 1])   #(B, mlp[2], 1, 1)
        global_feature = global_feature.expand(-1,-1,feature.size(2),-1)
        feature = torch.cat([feature, global_feature], dim=1)   #(B, 2*mlp[2], num_points, 1)

        feature = self.mlp2(feature)
        global_feature = F.max_pool2d(feature, kernel_size=[feature.size(2), 1])   #(B, mlp2[2], 1, 1)
